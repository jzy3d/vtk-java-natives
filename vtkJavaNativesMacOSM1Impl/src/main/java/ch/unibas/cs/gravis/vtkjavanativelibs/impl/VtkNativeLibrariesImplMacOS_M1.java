...
public class VtkNativeLibrariesImplMacOS_M1 implements VtkNativeLibrariesImpl {
    ...
    public VtkNativeLibrariesImplMacOS_M1() {
    }

    public String getName() {
        return "VtkNativeLibrariesMacOS_M1";
    }
    ...
    public List<URL> getJoglLibraries() {
        ...
        // If JOGL native libs stand in this JAR
        if(loadJOGLFromPlatformJar) {
        ...
        // If JOGL native libs stand in another JAR
        else {
          return libraryList
                  .stream()
                  .map(libraryName -> ClassLoader.getSystemResource("natives/macosx-universal/" + libraryName))
                  .collect(Collectors.toList());
        }
    }

    boolean loadJOGLFromPlatformJar = true;
